...
export default {
  ...
  methods: {
	...
	this.ruleForm.neirong = this.ruleForm.neirong.replace(reg,'../../../ssmli99j/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `xinwentoutiao/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.xinwentoutiaoCrossAddOrUpdateFlag = false;
                  this.parent.search();
