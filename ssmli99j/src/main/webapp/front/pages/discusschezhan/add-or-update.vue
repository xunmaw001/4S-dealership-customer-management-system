	...
	<view class="content">
		<form class="app-update-pv">
			...
			<!-- ${location} -->
				...
				<view :style='{"width":"180rpx","fontSize":"28rpx","color":"rgba(0, 204, 0, 1)","textAlign":"left"}' class="title">评论内容</view>
				<textarea :style='{"boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0)","borderColor":"rgba(0, 204, 0, 1)","borderRadius":"16rpx","color":"rgba(255, 69, 0, 1)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"280rpx"}' v-model="ruleForm.content" placeholder="评论内容"></textarea>
			</view>
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discusschezhan`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
					...
					return
																																																																				...
																																																																				if(this.ruleForm.id){
					await this.$api.update(`discusschezhan`, this.ruleForm);
				}else{
					await this.$api.add(`discusschezhan`, this.ruleForm);
				}
