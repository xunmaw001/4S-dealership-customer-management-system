	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`qichepinpai`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
																																																																if((!this.ruleForm.qichepinpai)){
					this.$utils.msg(`汽车品牌不能为空`);
					return
				}
																																				if(this.ruleForm.id){
					await this.$api.update(`qichepinpai`, this.ruleForm);
				}else{
					await this.$api.add(`qichepinpai`, this.ruleForm);
				}
